...
ifneq ($(NO_CACHE_FILTER),)
COMMON_ARGS += $(patsubst %,--no-cache-filter=%,$(NO_CACHE_FILTER))
endif
...
all: unit-tests kdev image-kdev lint
...
.PHONY: kdev-linux-amd64
kdev-linux-amd64: $(ARTIFACTS)/kdev-linux-amd64  ## Builds executable for kdev-linux-amd64.

.PHONY: kdev
kdev: kdev-linux-amd64  ## Builds executables for kdev.
...
.PHONY: lint
lint: lint-golangci-lint lint-gofumpt lint-govulncheck lint-markdown reuse  ## Run all linters for the project.

.PHONY: lint-fmt
lint-fmt: lint-golangci-lint-fmt  ## Run all linter formatters and fix up the source tree.

.PHONY: image-kdev
image-kdev:  ## Builds image for kdev.
	@$(MAKE) registry-$@ IMAGE_NAME="kdev"

.PHONY: reuse
reuse:  ## Check REUSE compliance.
	@docker run --rm -v $(PWD):/data:Z -w /data fsfe/reuse:latest lint
